...
class Tree {
    ...
    public ArrayList<Integer> serialize(Node root) {
     ...
     private void serialize(Node root, ArrayList<Integer> A) {
        if (root == null) {
            A.add(-1);
        } else {
            A.add(root.data);
    ...
    public Node deSerialize(ArrayList<Integer> A) {
        // code here
                index = 0; // Reset index before deserialization
        return deSerializeHelper(A);
}
 
 private int index; // Index for tracking position in the list during deserialization
