...
class Solution
{
    ...
    void treePath( int k, Node* root, vector<Node*>& path){
        if(root == NULL) return;
        path.push_back(root);
        if(root -> left == NULL && root -> right == NULL){
            if(path.size() > k) st.insert(path[path.size()-k-1]);
            path.pop_back();
            return;
        }
        treePath(k,root->left,path);
        treePath(k,root->right,path);
        path.pop_back();
        return;
    }
