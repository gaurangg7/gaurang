...
public:
    int globalDiff;
    Global() : globalDiff(INT_MIN) {}
};


int maxDiffUtil(Node* root, Global* obj) {
    if (root == nullptr)
        return INT_MAX;
    ...
    int currSmall = min(leftSmall, rightSmall);
    obj->globalDiff = max(obj->globalDiff, root->data - currSmall);

    return min(root->data, currSmall);
}
